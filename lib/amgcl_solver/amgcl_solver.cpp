...
namespace amgcl_solver {
	...
	QImage poisson(
		...
		ulong n = utility::generateCrsMatrix(target, source, mask, ptr, col, matrix_values,
		...
		for (size_t i = 0; i < pixels.size(); i++) {
			int r = utility::bound(int(result_r[i]));
			int g = utility::bound(int(result_g[i]));
			int b = utility::bound(int(result_b[i]));
