...
MultiCanvasView::MultiCanvasView(CanvasModel& model, QWidget* parent) :
	...
	grid->addWidget(new CanvasView(model, ImageType::IMG_MASK), 1, 1);

	grid->addWidget(new QLabel("Background"), 2, 0, Qt::AlignCenter);
	grid->addWidget(new CanvasView(model, ImageType::IMG_BG), 3, 0);

	grid->addWidget(new QLabel("Composed"), 2, 1, Qt::AlignCenter);
	grid->addWidget(new CanvasView(model, ImageType::IMG_COMPOSED), 3, 1);
